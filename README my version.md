# windows-dfir-lab72-powershell-reconnaissance-investigation
## Overview
PowerShell reconnaissance is the use of PowerShell commands to collect information about a Windows system, users, network, services, processes, and security configuration.

PowerShell itself is a legitimate administrative tool, so commands such as checking the hostname or listing running processes are not automatically malicious. The concern increases when multiple discovery commands are executed together, especially when they occur in an unusual context or are followed by other suspicious activity.

A typical reconnaissance sequence might look like:
...
Reconnaissance is often an early stage of an attack. An attacker may first learn what machine they are on, which users exist, what services are running, what network is available, and what security controls are present before deciding what to do next.

For a SOC analyst, the important question is therefore not simply:

"Was PowerShell used?"

Instead:

"What information was PowerShell attempting to discover, who executed the commands, and does the sequence fit normal administration or suspicious reconnaissance?"
The key distinction is:

PowerShell reconnaissance activity does not automatically mean compromise.

We need to correlate the commands with their execution context, frequency, user, timing, and surrounding activity.
...
- Understand how PowerShell can be used for Windows reconnaissance.
- Perform controlled system and host information discovery.
- Enumerate local users and security groups.
- Identify members of the local Administrators group.
- Enumerate running processes and associated process paths.
- Enumerate Windows services and their current status.
- Collect network configuration and interface information.
- Review active TCP connections and listening ports.
- Examine neighboring devices visible to the Windows host.
- Review Windows Firewall configuration.
- Check Microsoft Defender security status.
- Analyze PowerShell Script Block Logging through Event ID 4104.
- Investigate PowerShell process creation using Sysmon Event ID 1.
- Review Windows Security Event ID 4688 for process creation evidence.
- Investigate Sysmon Event ID 11 for controlled file-creation telemetry.
- Review Sysmon Event ID 3 for network activity.
- Correlate multiple telemetry sources into a chronological timeline.
- Identify telemetry gaps and limitations during the investigation.
- Distinguish reconnaissance activity from evidence of compromise.
- Produce an evidence-based final assessment without assuming malicious intent.

## Lab Scenario

PowerShell is commonly used for Windows administration, but the same capabilities can be abused for reconnaissance after an attacker gains access to a system. In this lab, a controlled PowerShell activity was performed to enumerate system information, users and groups, running processes, services, network connections, firewall settings, and Microsoft Defender status. The resulting activity was investigated using PowerShell Script Block Logging, Sysmon, Windows Security events, and filesystem evidence.

The investigation focused on:

- Identifying the information collected through PowerShell reconnaissance.
- Correlating PowerShell execution with process, network, and filesystem telemetry.
- Building a timeline of the reconnaissance activity.
- Assessing whether the observed behavior was suspicious or consistent with controlled administration.
- Identifying telemetry gaps and avoiding unsupported conclusions about compromise or malicious intent.
